...
app.get("/cookingClasses", async (req, res) => {
    ...
    let author = await db.query('SELECT * FROM users WHERE id = $1', [1]);
    res.render("blog.ejs", {heading: "Cooking Classes", posts: result.rows, author: author.rows[0], user: req.user});
})

app.get("/personalChefs", async (req, res) => {
    let result = await db.query(`SELECT * FROM blogposts where type= $1`, ['chefs']);
    let author = await db.query('SELECT * FROM users WHERE id = $1', [1]);
    res.render("blog.ejs", {heading: "Personal Chefs", posts: result.rows, author: author.rows[0],user: req.user});
})

app.get("/cookingTeamBuilding", async (req, res) => {
    let result = await db.query(`SELECT * FROM blogposts where type= $1`, ['teambuilding']);
    let author = await db.query('SELECT * FROM users WHERE id = $1', [1]);
    res.render("blog.ejs", {heading: "Cooking Team Building", posts: result.rows, author: author.rows[0],user: req.user});
})

app.get("/recipes", async (req, res) => {
    let result = await db.query(`SELECT * FROM blogposts where type= $1`, ['recipes']);
    let author = await db.query('SELECT * FROM users WHERE id = $1', [1]);
    res.render("blog.ejs", {heading: "Recipes", posts: result.rows, author: author.rows[0],user: req.user});
})
